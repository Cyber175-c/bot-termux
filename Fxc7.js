        ...
        const limitAdd = (sender) => {
			...
			case 'tp':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					anu = `https://mhankbarbar.tech/api/textpro?pack=${args[0]}&text=${body.slice(3+args[0].length+1)}&apiKey=${BarBarApi}`
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					anu = `https://mhankbarbar.tech/api/ephoto?pack=${args[0]}&text=${body.slice(3+args[0].length+1)}&apiKey=${BarBarApi}`
					voss = await fetch(anu)
					ftype = require('file-type')
					vuss = await ftype.fromStream(voss.body)
					//console.log(vuss)
		...
		case 'daftar':
			...
			if (!isPublic) return reply(mess.only.publikG)
                ...
                var found = false
					...
					await costum(help(prefix, instagram, yt, name, pushname2, user, limitt, uptime, jam, tanggal, Limit), text, FarhanGans, rmenu)
    				break 
    			case 'infobot':
